...
module.exports = async function version(filename, options={}) {
    ...
    const current = json['expo']['extra'][options.entry] || ''
    let updated, split
    if (options.replace) {
        // ET: added expo key here
        json['expo']['extra'][options.entry] = options.replace
        split = current.split('.')
        ...
        json['expo']['extra'][options.entry] = parseInt(split[0]) + '.' + parseInt(split[1]) + '.' + parseInt(split[2])
    }
    updated = json['expo']['extra'][options.entry]
